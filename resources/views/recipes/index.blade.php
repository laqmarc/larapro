<x-app-layout>
    ...
    <div class="py-8">
        <div class="mx-auto mt-4 mb-4 max-w-7xl space-y-8 px-4 sm:px-6 lg:px-8">
            <form method="GET" class="grid gap-6 rounded-lg bg-white p-6 shadow lg:grid-cols-4">
                ...
                <div>
                    ...
                    <select id="dish_type" name="dish_type" class="mt-2 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        @foreach ($dishTypes as $option)
                            <option value="{{ $option }}" @selected(request('dish_type') === $option)>{{ Str::title($option) }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div class="mb-4 lg:col-span-2">
                    <x-input-label value="Etiquetes dietétiques" />
                    <div class="fw mt-3 gap-2 ">
                        ...
                        <div class="flex flex-1 flex-col space-y-4 p-4">
                            <div class="flex items-start justify-between">
                                ...
                                <div class="flex flex-wrap justify-end gap-2">
                                    <span class="rounded-full bg-indigo-50 px-3 py-1 text-xs font-semibold text-indigo-600">{{ Str::title($recipe->difficulty ?? 'Qualsevol') }}</span>
                                    @if ($recipe->dish_type)
                                        <span class="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-600">{{ Str::title($recipe->dish_type) }}</span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="mt-auto space-y-2 text-sm text-gray-500">
                                ...
                                <p>
                                    ...
                                    <span class="mx-2 inline-block h-1 w-1 rounded-full bg-gray-300"></span>
                ...
                @empty
                    <div class="lg:col-span-3">
                        <div class="rounded-lg bg-white p-10 text-center shadow">
                            ...
                            <p class="mt-2 text-sm text-gray-600">Prova d'ajustar els filtres o torna-ho a comprovar mǸs tard.</p>
                        </div>
                    </div>
                @endforelse
            </div>
